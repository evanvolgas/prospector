...
class Portfolio(BaseModel):
    ...
    risk_tolerance: RiskTolerance = Field(default=RiskTolerance.MODERATE, 
                                        description="Client risk tolerance")
    ...
    def validate_total_value(cls, v, values):
        ...
        if 'positions' in values:
            expected = sum(pos.market_value for pos in values['positions'])
...
class RiskCalculation(BaseModel):
    ...
    advisor_id: str = Field(..., description="Advisor identifier")
    risk_number: int = Field(..., ge=1, le=99, description="Risk score (1-99)")
...
class PortfolioUpdate(BaseModel):
    ...
    recalculate_immediately: bool = Field(default=True, 
                                        description="Whether to trigger immediate risk calculation")
...
class MetricsSummary(BaseModel):
    ...
    avg_risk_number: float = Field(..., ge=0, le=99)
